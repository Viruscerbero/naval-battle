...
function Ship(location, orientation) {
	...
	/* Función privada para determinar las escalas en ancho y alto del sprite del buque.
	 * En el caso del buque diagonal, translada hacia la izquierda una cantidad de píxeles igual al doble del ancho de una celda
	*/
	function transform(orientation, shipLength) {

		// shipLength es la longitud del buque en celdas.
		// REFERENCE es 1.20 (corresponde al 120% del ancho o alto de una celda)
		// para compensar el estrechamiento de la imagen al escalarse según el ancho de las celdas
...
*/
Ship.prototype.setDirection = function setDirection(orientation) {
...
function EelWarship(location, orientation) {
	...
	if (!(this instanceof EelWarship)) {
		return new EelWarship(location, orientation);
	}

	Ship.call(this, location, orientation);

	let direction = this.setDirection(orientation);
...
function SharkWarship(location, orientation) {
	// Scope-Safe Constructor
	if (!(this instanceof SharkWarship)) {
		return new SharkWarship(location, orientation);
	}
